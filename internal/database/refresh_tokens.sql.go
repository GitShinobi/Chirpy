...
const addrevokedAt = `-- name: AddrevokedAt :exec
UPDATE refresh_tokens 
SET revoked_at = NOW(), updated_at = NOW() 
WHERE user_id = $1
`

func (q *Queries) AddrevokedAt(ctx context.Context, userID uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, addrevokedAt, userID)
	return err
}
